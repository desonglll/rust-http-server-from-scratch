...
/// - PageNotFoundHandler—To serve a 404 page
/// - OrderStatus—To load data read from a JSON file
///
pub trait Handler {
...
impl Handler for WebServiceHandler {
    fn handle(req: &HttpRequest) -> HttpResponse {
        // 这段代码的目的是从一个 http::httprequest::Resource 类型的枚举中解构出其中包含的值，并将其绑定到一个变量 s 上
        let http::httprequest::Resource::Path(s) = &req.resource;
        // Parse the URI
        let route: Vec<&str> = s.split("/").collect();
